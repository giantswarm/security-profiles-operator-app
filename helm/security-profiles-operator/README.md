...
| security-profiles-operator | object | `{"selinux":{"enable":true,"options":{"allowedTypes":["kernel_t","security_t"]}}}` | Values passed through to the upstream chart. For available options, see the [upstream values.yaml](https://github.com/kubernetes-sigs/security-profiles-operator/blob/v1.0.0/deploy/helm/values.yaml). |
